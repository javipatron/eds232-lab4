...
Model 1: The odds of a tree to be dead after 1 year of the fire ( yr1status = 1), increases multiplicatively by 1.0798 for each unit of CVS_percent (Crown Volume Scratch)
...
```{r}
#Model 1
ggplot(tree_test, aes(x = CVS_percent, y = yr1status)) +
  geom_point() +
...
```{r}
#Model 2
ggplot(tree_test, aes(x = BCHM_m, y = yr1status)) +
  geom_point() +
...
```{r}
#Model 3
ggplot(tree_test, aes(x = DBH_cm, y = yr1status)) +
  geom_point() +
...
*Response: The highest accurcy model comes from the multiple logistic regression model that has a mean accuarcy of 90.30% ´r max(models_accurarcy[,"Mean"])´.*
...
confusion_matrix <- confusionMatrix(data = predict_class,
...
*Response: The confusion Matrix is showing* that general "big" numbers lay within the "good" predictions. Which are the True Positive (TP) with 16486 observations and 6312 of True Negative (TN). However between the "bad" numbers there is a significant increase on false positive (FP) of 1,597 observations saying that the tree is predicted alive, but the true value is that is dead. Consequentenly for the remaining 851 observations they are False Negative (FN) meaning that the predicted value is Dead, but the true value is Alive.
...
\*Response: The overall accuracy of the model is 90.30%. There is a formula to calculate the overall accuracy:

$Overall Accuarcy = TP + TN / total observations$
